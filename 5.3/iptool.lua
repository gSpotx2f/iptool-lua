...
local function int_to_ip(number)
    local octets = {}
    for i = 1, 4 do
        table.insert(octets, 1, tostring(number & 255))
        number = number >> 8
    end
...
local function count_righthand_zero_bits(number, bits)
    return (number == 0) and bits or math.min(bits, bit_length((~ number) & (number - 1)))
end
...
local function hosts_from_network(network)
    ...
    local last_ip = network_address + ((1 << (32 - prefixlen)) - 2)
    local ret_val
...
local function get_supernet(network, new_prefix)
    ...
    local netmask = ipv4_capacity ~ (ipv4_capacity >> prefixlen)
